# -*- coding: utf-8 -*-
"""
US101 highway co-simulation scenario **with DAROM merging control**.
...
def run_scenario(opt, scenario_params):
    ...
    try:
        ...
        _default_model = os.path.join(
            ...
            'models', 'GRU-uniform-delay', 'GRU-uniform-delay_best.zip')
        ...
        # ── State estimator / Birdseye camera setup (deferred) ───────
        # Will be initialized once ego vehicle appears in CARLA
        birdseye_cam = None
        state_estimator = None
        if not opt.state_estimator:
            birdseye_params = scenario_params.get('birdseye_camera', {})
        else:
            birdseye_params = None
            # Get YOLO model path for state estimator
            state_est_cfg = scenario_params.get('state_estimator', {})
            yolo_cfg = state_est_cfg.get('yolo', {})
            yolo_model = yolo_cfg.get('model', 'yolov5m.pt')
            # If not absolute path, assume it's in OpenCDA root
            if not os.path.isabs(yolo_model):
                yolo_model = os.path.join(
                    os.path.dirname(__file__), '..', '..', yolo_model)
            yolo_model_path = os.path.expanduser(yolo_model)

        # ── Performance evaluator setup ───────────────────────────────
        if opt.evaluation:
                ...
                if carla_id is not None:
                    ...
                    if ego_actor is not None:
                        if opt.state_estimator:
                            ...
                            # Use birdseye camera
                            camera_cfg = birdseye_params.get('camera', {})
                            visualizer_cfg = birdseye_params.get('visualizer', {})
                            birdseye_cam = BirdseyeCamera(
                                scenario_manager.world, ego_actor,
                                width=camera_cfg.get('width', 800),
                                height=camera_cfg.get('height', 600),
                                z=camera_cfg.get('z', 50),
                                fov=camera_cfg.get('fov', 90),
                                window_title=visualizer_cfg.get('window_name', "Bird's-Eye View"))
